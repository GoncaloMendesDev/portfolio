...
// Generate random letters for the background
const LETTERS = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789<>[]{}".split("");
const FALLING_LETTERS = Array.from({ length: 30 }, (_, i) => ({
  id: i,
  char: LETTERS[Math.floor(Math.random() * LETTERS.length)],
  left: Math.random() * 100,
  delay: Math.random() * 5,
  duration: Math.random() * 10 + 10, // Random duration between 10-20s
  fontSize: Math.random() * 16 + 12 // Random size between 12-28px
}));
...
function App() {
  ...
  const stars = Array.from({ length: 50 }, (_, i) => ({
    ...
    top: Math.random() * 100,
    size: Math.random() * 3,
    delay: Math.random() * 3 // Add random delay for out-of-sync animation
  }));
    ...
    <div className={`min-h-screen ${darkMode ? 'dark bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      ...
      <section id="home" className="pt-20 md:pt-32 pb-20 relative overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          {/* Falling Letters */}
          {FALLING_LETTERS.map((letter) => (
            <div
              key={letter.id}
              className="absolute falling-letter text-purple-500/20"
              style={{
                left: `${letter.left}%`,
      ...
      <section id="about" className="py-20 bg-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-gray-700 rounded-lg p-8 shadow-xl transform hover:scale-105 transition-transform duration-300 led-glow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {PROJECTS.map((project, index) => (
              <a
                ...
                className="block bg-gray-800 rounded-lg overflow-hidden shadow-xl transform hover:scale-105 transition-transform duration-300 led-glow"
              >
      ...
      <section id="cv" className="py-20 bg-gray-800">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-gray-700 rounded-lg p-8 shadow-xl led-glow">
